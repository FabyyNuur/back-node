...
- Spécification OpenAPI (JSON) : `http://localhost:4000/api-docs.json`

### Tester les routes protégées

1. Lancez la route de login `POST /api/users/login` dans Swagger.
2. Récupérez le token JWT dans la réponse.
3. Cliquez sur **Authorize** (en haut à droite) puis collez : `Bearer <votre_token>`.
4. Vous pouvez ensuite tester toutes les routes protégées.
