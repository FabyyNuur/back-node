...
const userCtrl = {
    list: (req, res) => {
        try {
            const rows = db.prepare(`SELECT id, email, name, role, is_active FROM users`).all();
    ...
    add: (req, res) => {
        ...
        try {
            ...
            const result = db.prepare(`INSERT INTO users (email, password, name, role, is_active) VALUES (?, ?, ?, ?, ?)`).run(
                email, hashedPassword, name, userRole, activeInsert === null ? 1 : activeInsert
            );
    ...
    update: (req, res) => {
        ...
        try {
            ...
            if (req.user && Number(req.user.id) === Number(id) && nextIsActive === 0) {
            ...
            if (password) {
                const hashedPassword = bcrypt.hashSync(password, 10);
                result = db.prepare(`UPDATE users SET email = ?, password = ?, name = ?, role = ?, is_active = ? WHERE id = ?`).run(
                    email, hashedPassword, name, userRole, nextIsActive, id
                );
            } else {
                result = db.prepare(`UPDATE users SET email = ?, name = ?, role = ?, is_active = ? WHERE id = ?`).run(
                    email, name, userRole, nextIsActive, id
                );
            }
            if (result.changes === 0) {
                return res.status(404).json({ message: "Utilisateur introuvable." });
